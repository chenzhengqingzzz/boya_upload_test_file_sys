# -*- coding: utf-8 -*-
"""
Created on Mon Nov 11 14:30:26 2024
...
def get_file_data(file_path,testname2):
    ...
    f.close()
    for line in lines:
        searchObj = re.match(r".*read_shmoo_margin_test.* (?P<tv>.*) ns, (?P<vcc>.*) V, (?P<MHz>.*) MHz, (?P<result>.*)!!", line)
        if not searchObj:
            ...
            elif 'readv average value' in line:
                searchObj = re.match(r".*readv average value = (?P<rdv>.*) uA", line)
                rdv = float(str(searchObj['rdv']))
        else:
            if testname == 0:
                testname = testname2
                DATAlist.update({testname:{}})
